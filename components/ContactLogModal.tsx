...
export const ContactLogModal: React.FC<{ 
  ...
  const handleCloseAttempt = async () => {
    // Logic updated per request: Trigger warning if note is empty regardless of reservation status
    // "所有立刻預約 關閉 也會觸發別讓努力白費"
    // Also, if product ID is entered but not saved, it's worth warning.
    const isDirty = note.trim() || (isReservation && (quoteAmount || productId));

    // If user hasn't written a note (or basic reservation details) and tries to close, trigger warning.
    // The tutorial 'CONTACT_LOG_MISSING' logic is: "You opened the window but haven't recorded anything."
    // So if !note.trim(), we trigger it.
